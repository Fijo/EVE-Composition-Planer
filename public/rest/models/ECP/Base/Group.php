...
/**
 * Base class that represents a row from the 'gr0up' table.
 *
 * 
 *
...
abstract class Group implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\ECP\\Map\\GroupTableMap';
    ...
    /**
     * Initializes internal state of ECP\Base\Group object.
     */
     ...
     * @return $this|\ECP\Group The current object (for fluent API support)
     */
    ...
    public function setLastcomputed($v)
    {
        ...
        if ($this->lastcomputed !== null || $dt !== null) {
            if ($this->lastcomputed === null || $dt === null || $dt->format("Y-m-d H:i:s") !== $this->lastcomputed->format("Y-m-d H:i:s")) {
                ...
                $this->modifiedColumns[GroupTableMap::COL_LASTCOMPUTED] = true;
            }
    ...
    public function setLastmodified($v)
    {
        ...
        if ($this->lastmodified !== null || $dt !== null) {
            if ($this->lastmodified === null || $dt === null || $dt->format("Y-m-d H:i:s") !== $this->lastmodified->format("Y-m-d H:i:s")) {
                ...
                $this->modifiedColumns[GroupTableMap::COL_LASTMODIFIED] = true;
            }
        } // if either are not null
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GroupTableMap::translateFieldName('Lastcomputed', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->lastcomputed = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : GroupTableMap::translateFieldName('Lastmodified', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\ECP\\Group'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(GroupTableMap::COL_LASTCOMPUTED)) {
            $modifiedColumns[':p' . $index++]  = 'lastComputed';
        }
        if ($this->isColumnModified(GroupTableMap::COL_LASTMODIFIED)) {
            $modifiedColumns[':p' . $index++]  = 'lastModified';
        }

        $sql = sprintf(
            'INSERT INTO gr0up (%s) VALUES (%s)',
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setLastcomputed($arr[$keys[2]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\ECP\Group The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GroupTableMap::COL_LASTCOMPUTED)) {
            $criteria->add(GroupTableMap::COL_LASTCOMPUTED, $this->lastcomputed);
        }
        if ($this->isColumnModified(GroupTableMap::COL_LASTMODIFIED)) {
            $criteria->add(GroupTableMap::COL_LASTMODIFIED, $this->lastmodified);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \ECP\Group (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \ECP\Group Clone of current object.
     * @throws PropelException
     */
    ...
    public function setGroupEvepeople(Collection $groupEvepeople, ConnectionInterface $con = null)
    {
        ...
        foreach ($groupEvepeopleToDelete as $groupEvePersonRemoved) {
            $groupEvePersonRemoved->setGroup(null);
        }
     ...
     * @return ObjectCollection|ChildGroupEvePerson[] List of ChildGroupEvePerson objects
     */
    public function getGroupEvepeopleJoinGroupPersonType(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildGroupEvePersonQuery::create(null, $criteria);
    ...
    public function setGrouppeople(Collection $grouppeople, ConnectionInterface $con = null)
    {
        ...
        foreach ($grouppeopleToDelete as $groupPersonRemoved) {
            $groupPersonRemoved->setGroup(null);
        }
    ...
    public function setGroupAccesses(Collection $groupAccesses, ConnectionInterface $con = null)
    {
        ...
        foreach ($groupAccessesToDelete as $groupAccessRemoved) {
            $groupAccessRemoved->setGroup(null);
        }
     ...
     * @return ObjectCollection|ChildGroupAccess[] List of ChildGroupAccess objects
     */
    public function getGroupAccessesJoinEntityType(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildGroupAccessQuery::create(null, $criteria);
        $query->joinWith('EntityType', $joinBehavior);
